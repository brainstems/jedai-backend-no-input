...
async def test_get_daily_event():
    ...
    database_operations.get_daily_event = AsyncMock(
        return_value={
            "Items": [
                {
    ...
    expected_fields = ["end_ts", "contextPrompt", "assistantPrompt", "team", "start_ts"]
    # Call the function and verify the result
    with patch.object(
        DatabaseOperations, "get_daily_event", database_operations.get_daily_event
    ):
...
async def test_get_daily_event_no_items():
    ...
    database_operations = DatabaseOperations()
    prediction_service = PredictionService()
    database_operations.get_daily_event = AsyncMock(return_value={"Items": []})
    with patch.object(
        DatabaseOperations, "get_daily_event", database_operations.get_daily_event
    ):
        result = await prediction_service.get_daily_event()
...
async def test_get_next_event():
    database_operations = DatabaseOperations()
    prediction_service = PredictionService()
    database_operations.get_next_event = AsyncMock(
        return_value={
            "Items": [
                {
                    "start_ts": "2022-02-15T17:00:00+00:00",
    ...
    # Call the function and verify the result
    expected_fields = ["start_date", "team"]

    # Patch the method to use the mock
    with patch.object(
        DatabaseOperations, "get_next_event", database_operations.get_next_event
    ):
        # Call the method
...
async def test_available_to_predict_no_items():
    ...
    with patch.object(
        DatabaseOperations,
        "available_to_predict",
        database_operations.available_to_predict,
    ):
        # Call the method
...
async def test_available_to_predict_with_items():
    ...
    # Hardcode the value directly in the method
    prediction_service.get_daily_event = AsyncMock(return_value={"team": team})
    database_operations.available_to_predict = AsyncMock(
        return_value={"Items": [{"address": address, "team": "Team A"}]}
    )
    with patch.object(
        DatabaseOperations,
        "available_to_predict",
